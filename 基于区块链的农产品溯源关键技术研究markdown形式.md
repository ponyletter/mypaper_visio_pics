...
## 1.1 研究背景与意义
随着经济社会的快速进步和居民生活品质的提升，公众对于食品安全的关切度也随之显著增加。农产品作为食品的重要组成部分，其质量和安全直接影响到消费者的健康和生命安全[1]。如图1-1所示，近年来我国苹果产量持续保持高位，巨大的市场体量背后是对产品质量安全与品牌信誉的更高要求。然而，食品安全事件的接连发生，如2019年染色土鸡蛋，2022年问题酸菜，2024年煤油车混拉食用油，2025黄焖鸡事件等，削弱了大众对市场上食用农产品的信任度。因此，如何提高食用农产品的安全生产水平、加强质量管理、畅通社会监管途径，并最终实现农产品的可信追溯，已成为学术界和产业界共同关注的焦点议题[2]。传统的农产品溯源系统普遍依赖中心化数据进行信息存储与访问，这种模式虽然在一定程度上实现了信息化管理，但其固有的信息不透明、数据易被篡改以及信任机制缺失等问题，使其难以满足现代农业生产的高标准和消费者对食品安全信息的高要求[3]。

![图1-1  2014年-2024年中国苹果产量及增速情况](<第一章/图1-1  2014年-2024年中国苹果产量及增速情况.png>)
...
![图2-1  区块链区块与链式结构原理图](<第二章/图2-1  区块链区块与链式结构原理图.png>)
*图2-1  区块链区块与链式结构原理图*
...
![图2-2  Hyperledger Fabric联盟链结构](<第二章/图2-2  Hyperledger Fabric联盟链结构.png>)
...
![图2-3  使用Hyperledger Fabric实现的业务网络](<第二章/图2-3  使用Hyperledger Fabric实现的业务网络.png>)
...
![图3-1  农产品中心化溯源模式](<第三章/图3-1  农产品中心化溯源模式.png>)
*图3-1  农产品中心化溯源模式*
...
![图3-2  农产品供应链溯源流程图](<第三章/图3-2  农产品供应链溯源流程图.png>)
*图3-2  农产品供应链溯源流程图*
...
![图3-3  链上链下双存储数据管理模型](<第三章/图3-3  链上链下双存储数据管理模型.png>)
*图3-3  链上链下双存储数据管理模型*
...
此外，结合区块链与关系型数据库的数据管理策略旨在发挥两者的技术优势。链上存储聚焦于关键存证数据，链下MySQL数据库则承担大容量辅助业务数据的存储任务，并通过IPFS内容寻址技术实现文件的分布式托管。为保障网络环境的高效协作与隐私隔离，Fabric底层网络采用多通道架构设计。系统不仅构建了基础公共通道用于全局信息的广播，还针对种植、加工、储运与销售环节分别划分了独立的业务通道。同时，结合多组织背书策略，要求核心交易必须获取多个利益相关节点的共同签名方可生效，从而在底层架构上形成完整的证据链条与信任闭闭环。
...
![图3-4  农产品可信溯源系统整体架构图](<第三章/图3-4  农产品可信溯源系统整体架构图.png>)
*图3-4  农产品可信溯源系统整体架构图*
...
![图3-5  基于区块链的农产品溯源模型](<第三章/图3-5  基于区块链的农产品溯源模型.png>)
*图3-5  基于区块链的农产品溯源模型*
...
![图4-1  Fabric 平台组件间交互逻辑图](<第四章/图4-1 Fabric 平台组件间交互逻辑图.png>)
*图4-1  Fabric 平台组件间交互逻辑图*

在哈希计算方面，BCCSP支持标准SHA系列算法，其核心公式为：
H(x)=hash(x)    （4-1）
其中x表示输入数据。这种设计不仅确保了数据完整性验证的可靠性，还为引入其他哈希算法提供了扩展空间。在Fabric中，成员服务提供者(Membership Service Provider, MSP)通过调用BCCSP接口完成身份认证和权限管理等关键功能，使得整个系统的安全性得以保障。
...
(rⓜ,s)=sign(k_privⓜ,m)    （4-2）
，其中k_priv是私钥，m是消息摘要。而PKCS11模块则通过HSM提供更高安全级别的加密服务，特别适用于需要强密钥保护的场景。这种多实现支持的架构设计，使得Fabric能够适应从普通企业级应用到高敏感度金融系统的多样化需求。

在将国密算法嵌入Fabric时，面临的主要挑战之一是算法兼容性问题。国密算法如SM2、SM3与现有国际标准算法存在显著差异，尤其是在公钥生成和签名验证流程上。例如，SM2签名算法基于中国国家密码管理局制定的椭圆曲线参数，其签名公式为：
r=e+x_1 n    （4-3）
，s=k^(-1) (m+d_A r)n，其中d_A是用户私钥，k是随机数，m是消息摘要。由于这些参数与国际标准算法不一致，直接替换可能导致系统兼容性问题。国密算法通常具有较高的计算复杂度，特别是在大素数运算和模逆运算方面，这对Fabric的性能优化提出了更高要求。
...
![图4-2  BCCSP 内部工厂模式逻辑架构图](<第四章/图4-2 BCCSP 内部工厂模式逻辑架构图.png>)
...
本研究采取了系统化的设计思路。核心是通过扩展 BCCSP 接口，创建新的国密提供者（GM Provider），使其能够无缝支持国密算法操作。这一过程涉及到从底层算法库的集成、BCCSP 适配层的构建，到上层核心组件的全面适配。国密算法嵌入的总体设计思路如图4-3所示。

![图4-3  Fabric平台国密算法嵌入设计思路](<第四章/图4-3  Fabric平台国密算法嵌入设计思路.png>)
*图4-3  Fabric平台国密算法嵌入设计思路*

在实现过程中，本研究在 BCCSP 初始化阶段引入了国密算法的注册逻辑。具体而言，当系统配置指定使用国密提供者时，系统会加载国密底层基础库，并将 SM2 签名者、SM3 哈希者及 SM4 加解密者注入到 BCCSP 的实现类映射表中。这种基于插件化的设计方案，最大限度地减少了对 Fabric 核心代码的侵入性修改。BCCSP 国密算法接口实现示意如图4-4所示。

![图4-4  BCCSP 国密算法接口实现示意](<第四章/图4-4  BCCSP 国密算法接口实现示意.png>)
...
完成了GMProvider的开发后，接下来的关键环节是对Fabric平台的核心组件进行修改和适配，以使其能够正确地识别、加载并使用这个新的国密Provider。这主要涉及到Peer节点、Orderer节点以及MSP等组件。在Peer和Orderer组件中，需要找到并修改直接调用BCCSP接口进行加密、签名、验证或哈希计算的代码逻辑，使其能够根据配置选择并调用GMProvider提供的国密算法实现。在处理交易提案、进行背书签名、验证区块完整性等环节，都需要确保使用的是国密算法。MSP的核心职责是管理网络成员的身份和验证数字证书凭证，因此需要对MSP进行改造，使其能够支持解析和验证符合国密标准的数字证书，确保能够处理SM2公钥及SM3withSM2签名算法，并能从中正确提取用于身份认证和权限控制的各类信息。此外，还需要修改MSP的相关配置参数如config.yaml文件，以便在Fabric网络启动时能够正确加载国密算法相关的根证书、中间证书以及吊销列表等，并指定使用GMProvider作为密码服务提供者。

数字证书是Fabric平台身份管理和安全通信的基石。因此要实现国密算法的全面支持，必须对证书生成工具cryptogen和证书颁发机构Fabric CA进行相应的改造。cryptogen工具主要用于在开发和测试环境中批量生成网络组织的初始证书和密钥材料。我们需要修改其代码逻辑，使其能够根据配置文件中的指示，生成使用SM2算法的密钥对，并签发符合国密规范的X.509证书，这通常涉及到更新cryptogen的证书模板和签名算法选项。Fabric CA是一个功能更完善的、可用于生产环境的证书颁发机构，除了需要让Fabric CA的底层BCCSP能够使用国密算法外，还需要修改其证书签发、续期、吊销以及查询等核心业务逻辑，确保其能够完整支持国密证书的整个生命周期管理。这可能需要调整Fabric CA的数据库表结构以存储国密证书特有的信息，并更新其API接口和配置文件以反映对国密算法的支持。图4-7展示了在执行核心交易流转时的国密底层签名与背书通信时序过程。

![图4-7  基于国密算法的核心交易背书与上链时序图](<第四章/图4-7  基于国密算法的核心交易背书与上链时序图.png>)
...
![图4-8  BCCSP上的SM2算法接口有用性和有效性验证结果](<第四章/图4-8 BCCSP上的SM2算法接口有用性和有效性验证结果.png>)
...
![图5-1  溯源系统底层区块链网络及相关服务启动界面](<第五章/图5-1  溯源系统底层区块链网络及相关服务启动界面.png>)
...
![图5-2  溯源系统用例图](<第五章/图5-2  溯源系统用例图.png>)
*图5-2  溯源系统用例图*
...
![图5-3  系统功能模块](<第五章/图5-3  系统功能模块.png>)
*图5-3  系统功能模块*
...
**表5-3 核心业务数据结构与链上链下存储映射关系**
| 业务节点 | 核心字段名         | 数据类型  | 字段说明与标准映射                     | 上链状态     |
| :------- | :----------------- | :-------- | :------------------------------------- | :----------- |
| 种植源头 | batch_id           | String    | 农产品唯一批次号（基于追溯规范生成）   | 是           |
| 种植源头 | origin_cert_hash   | String    | 地理标志认证证书的SM3哈希摘要          | 是           |
| 种植源头 | env_parameters     | JSON      | 包含土壤pH值、灌溉水质等标准化种植参数 | 否（存链下） |
| 种植源头 | operator_id        | String    | 种植基地责任人国密数字身份标识         | 是           |
| 加工质检 | process_line_no    | String    | 标准化分拣与包装流水线编号             | 否（存链下） |
| 加工质检 | quality_report_cid | String    | 质检报告原件在IPFS网络的唯一内容标识符 | 是           |
| 加工质检 | pack_timestamp     | Timestamp | 包装赋码的时间戳                       | 是           |
| 冷链物流 | trans_order_id     | String    | 运输流转单号                           | 是           |
| 冷链物流 | cold_chain_temp    | Float     | 运输途中的冷链温度记录                 | 否（存链下） |
| 冷链物流 | vehicle_route      | String    | 脱敏后的运输轨迹坐标集                 | 否（存链下） |
| 终端销售 | retailer_code      | String    | 销售商统一社会信用代码标识             | 是           |
| 终端销售 | shelf_time         | Timestamp | 终端商超上架时间戳                     | 是           |
...
![图5-4 (a)](<第五章/图5-4  系统登录与注册页面 (a) 授权用户安全登录界面.png>)
*(a) 授权用户安全登录界面*
![图5-4 (b)](<第五章/图5-4  系统登录与注册页面 (b) 新增供应链角色注册申请.png>)
*(b) 新增供应链角色注册申请*
...
![图5-5 (a)](<第五章/图5-5  溯源系统工作界面 (a) 基地源头种植参数录入.png>)
*(a) 基地源头种植参数录入*
![图5-5 (b)](<第五章/图5-5  溯源系统工作界面 (b) 产线加工包装批次创建.png>)
*(b) 产线加工包装批次创建*

![图5-5 (c)](<第五章/图5-5  溯源系统工作界面 质检记录表单 (c) 权威质检报告文件上链.png>)
*(c) 权威质检报告文件上链*
![图5-5 (d)](<第五章/图5-5  溯源系统工作界面 (d) 运输节点与环境监测登记.png>)
*(d) 运输节点与环境监测登记*
...
![图5-6 (a)](<第五章/图5-6  溯源数据宏观展示与底层节点监控系统(a) 宏观业务数据监控大屏.png>)
*(a) 宏观业务数据监控大屏*
![图5-6 (b)](<第五章/图5-6  溯源数据宏观展示与底层节点监控系统 (b) 底层区块及交易实时浏览器.png>)
*(b) 底层区块及交易实时浏览器*
...
![图5-7 (a)](<第五章/图5-7  小程序端全链路溯源核验界面(a) 小程序消费者扫码入口.png>)
*(a) 小程序消费者扫码入口*
![图5-7 (b)](<第五章/图5-7  小程序端全链路溯源核验界面 (b) 小程序时间轴防伪溯源凭证.png>)
*(b) 小程序时间轴防伪溯源凭证*
...
![图5-8](<第五章/图5-8  Web端溯源节点检索展示 图5-8  Web端溯源节点检索展示.png>)
*图5-8  Web端溯源节点检索展示*
...
![图5-10](<第五章/图5-10  TPS并发曲线延迟并发曲线.png>)
*图5-10  TPS并发曲线延迟并发曲线*
...
[7] Tabatabaei Mohammad Hossein,Vitenberg Roman,Veeraraghavan Narasimha Raghavan. Understanding blockchain: Definitions, architecture, design, and system comparison[J].Computer Science Review,2023,50.
...
[27] W Xie, X Zheng, X Lu, X Lin, X Fan. Agricultural product traceability system based on blockchain technology. 2019 IEEE Intl Conf on Parallel & Distributed Processing with Applications, 2019.
[28] 周文欣.基于区块链技术的农产品溯源系统面临的问题及对策[J].南方农机,2021.
[29] 王蒙蒙.基于区块链的生鲜农产品供应链系统研究[D].江西理工大学,2023.
[30] 史亮 张复宏 刘文军.基于区块链的果蔬农产品追溯体系研究[J].农村经济与科技,2019.
[31] 赵超.区块链技术与农产品供应链融合发展研究[J].新疆财经,2020.
[32] 高潇.基于区块链的产品溯源认证技术研究[D].临沂大学,2023.
[33] 王亚伟.区块链技术在食品可信溯源中的应用研究[D].天津科技大学,2022.
[34] 史亮.基于区块链+物联网的果蔬农产品供应链追溯体系研究[D].山东农业大学,2020.
[35] 于合龙 陈邦越 徐大明 杨信廷 孙传恒.基于区块链的水稻供应链溯源信息保护模型研究[J].农业机械学报,2020.
[36] 魏文彦.基于区块链的农产品安全溯源平台系统设计与实现[D].武汉轻工大学,2022.
[37] 冯家琦.基于区块链的数据溯源方法研究与应用[D].重庆邮电大学,2021.
[38] 胡俊辉.基于区块链的水产品交易溯源系统研究与实现[D].上海海洋大学,2022.
[39] 陈锦雯 罗得寸 唐呈俊 唐晨钧 丁勇.基于区块链的农业物联网可信溯源体系[J].信息安全学报,2022.
[40] 陈少军.基于区块链的供应链信息可溯源方法研究[J].信息技术与信息化,2020.
[41] 孙传恒, 朱文颖, 邢斌, 等. 中国农产品质量安全可追溯体系发展历程与展望[J]. Transactions of the Chinese Society of Agricultural Engineering, 2025, 41(12).
[42] Nakamoto S. Bitcoin: A peer-to-peer electronic cash system[EB/OL].(2008).
[43] 胡祥培,都牧,孔祥维,等.基于区块链的农产品供应链溯源研究综述[J].管理科学学报,2024,27(05):1-12.
[44] 唐豪, 易文龙, 赵应丁, 等. 基于区块链的农产品可信检测数据存储方法[J]. 科学技术与工程, 2022, 22(24).
[45] 周颖玉,王海英,柯平,等.基于区块链的开放政府数据“链上链下-双存储”共享模型研究[J].情报杂志,2023,42(09):188-195.
[46] 张新, 彭祥贞, 李悦, 等. 基于可信区块链和可信标识的粮油食品全供应链信息追溯模型[J]. 农业大数据学报, 2022, 4(1): 35-48.
[47] Xu W ,Guo H ,Zhang X , et al.Research on a Blockchain-Based Quality and Safety Traceability System for Hymenopellis raphanipes[J].Sustainability,2025,17(16).
[48] Malavathula N, Shaik S, Nayanagar S C R, et al. Decentralized traceability and direct marketing of agricultural supply chain[C]//AIP Conference Proceedings, 2025.
[49] Yuan Z ,Xuyang W ,Hongyi G , et al.A Blockchain-Based Traceability Model for Grain and Oil Food Supply Chain.[J].Foods (Basel, Switzerland),2023,12(17):DOI:10.3390/FOODS12173235.
[50] Xin Z ,Yue L ,Xiangzhen P , et al.Information Traceability Model for the Grain and Oil Food Supply Chain Based on Trusted Identification and Trusted Blockchain[J].International Journal of Environmental Research and Public Health,2022,19(11):6594-6594.
[51] Xu, J., Zhao, Y., Chen, H. & Deng, W. ABC-GSPBFT: PBFT with grouping score mechanism and optimized consensus process for flight operation data-sharing. Inf. Sci. 624, 110–127.
[52] Hu, X., Yi, H., Zhang, W., Du, Y. (2024). Optimizing the SM4 Encryption Algorithm for Blockchain Security. In: Zhu, J., Wu, Q., Ding, Y., Song, X., Lu, Z. (eds) Blockchain Technology and Application. CBCS 2023. Communications in Computer and Information Science, vol 2098. Springer, Singapore.
